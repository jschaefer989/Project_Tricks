...
export default class Enemy extends Character {
  ...
  load(gameManager: GameManager, data?: EnemyData): void {
    ...
    this.hand = data?.hand
      ? data.hand.map((cardData) => Card.load(gameManager, cardData))
      : [];
    this.deck = data?.deck
      ? data.deck.map((cardData) => Card.load(gameManager, cardData))
      : [];
    this.discardPile = data?.discardPile
      ? data.discardPile.map((cardData) => Card.load(gameManager, cardData))
      : [];
