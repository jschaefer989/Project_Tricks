...
export default class GlowAnimation extends Animation {
  private originalColors: Map<string, [number, number, number, number]> =
    new Map();
  ...
  constructor(
    ...
    this.pulsePeriod =
      constructionOptions?.glowPeriodSeconds ?? this.pulsePeriod;
  ...
  updateAnimation(deltaTime: number): void {
    ...
    const intensity =
      0.5 + 0.5 * Math.sin((this.animElapsed / this.pulsePeriod) * Math.PI * 2);
